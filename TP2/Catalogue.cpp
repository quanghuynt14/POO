...
void Catalogue::RechercheSimple(const char *Start, const char *Goal) const
{
    ...
    while (x != NULL) {
        if (strcmp(x->getTrajet()->getDepart(), Start) == 0 && strcmp(x->getTrajet()->getArrivee(), Goal) == 0) {
            nombreParcours++;
...
void Catalogue::BFS(const Trajet *x, bool *v, const char *Goal, Liste *Chemin, int &nbParcours) const
{
    if (strcmp(x->getArrivee(), Goal) == 0) {
        ...
        while (y != NULL) {
            if (strcmp(x->getArrivee(), y->getTrajet()->getDepart()) == 0 && v[trajets->getPosition(y)] == false) {
